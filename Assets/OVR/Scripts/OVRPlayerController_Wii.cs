...
/// by the player.
/// </summary>
public class OVRPlayerController_Wii : MonoBehaviour
{
	[DllImport ("UniWii")]
	...
	public virtual void UpdateMovement()
	{
		...
		//WiiMote
		if(wiimote_getButtonUp(wiiCount))
		{
			moveForward = true;
			dpad_move = true;
		}
		if(wiimote_getButtonDown(wiiCount))
		{
			moveBack = true;
			dpad_move = true;
		}
		if(wiimote_getButtonLeft(wiiCount))
		{
			moveLeft = true;
			dpad_move = true;
		}
		if(wiimote_getButtonRight(wiiCount))
		{
			moveRight = true;
			dpad_move = true;
		}
		...
		if (Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift))
			...
			// set this to 1 so you're not running around the store like a maniac if you're using the wiimote.
			moveInfluence *= 1.0f;
